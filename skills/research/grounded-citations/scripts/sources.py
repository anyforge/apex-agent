...
Ledger path resolution (first wins):
  --ledger PATH
  $APEX_CITATION_LEDGER
  ~/.apex-agent/cache/citations/ledger.json
"""
...
from _apex_home import get_apex_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("APEX_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_apex_home() / "cache" / "citations" / "ledger.json"
