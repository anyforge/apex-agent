...
Ledger location: `~/.apex-agent/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `APEX_CITATION_LEDGER`.

## How to Run

```bash
S=~/.apex-agent/skills/research/grounded-citations/scripts/sources.py
...
install the official social-media/reddit-reading skill or
install the official research/rss-feeds skill before using them.
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `APEX_CITATION_LEDGER`) if their
