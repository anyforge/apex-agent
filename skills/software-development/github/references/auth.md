...
- Give it a name like "apex-agent"
- Select scopes:
...
- Give it a title like "apex-agent-<machine-name>"
...
uv run python "~/.apex-agent/skills/software-development/github/scripts/git-credential-token.py"
```
...
elif _apex_env="~/.apex-agent/.env"; [ -f "$_apex_env" ] && grep -q "^GITHUB_TOKEN=" "$_apex_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_apex_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python "~/.apex-agent/skills/software-development/github/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
