#!/usr/bin/env bash
# GitHub environment detection helper.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _apex_env="~/.apex-agent/.env"; [ -f "$_apex_env" ] && grep -q "^GITHUB_TOKEN=" "$_apex_env" 2>/dev/null; then
    GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_apex_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    if [ -n "$GITHUB_TOKEN" ]; then
        GH_AUTH_METHOD="curl"
    fi
elif [ -f "$HOME/.git-credentials" ]; then
    GITHUB_TOKEN=$(uv run python3 "~/.apex-agent/skills/software-development/github/scripts/git-credential-token.py")
    if [ -n "$GITHUB_TOKEN" ]; then
        GH_AUTH_METHOD="curl"
    fi
fi
...
[ "$GH_AUTH_METHOD" = "none" ] && echo "⚠ Not authenticated — see the auth setup below"
